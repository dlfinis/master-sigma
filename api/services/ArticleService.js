...
module.exports = {
  _accentFold : function (str) {
    ...
    for (var i = 0; i < str.length; i++) {
      if(accent_map[str.charAt(i)])
        ret += accent_map[str.charAt(i)];
      else
        if(accent_map[str.charAt(i).toLowerCase()] && accent_map[str.charAt(i).toLowerCase()].toUpperCase())
          ret += accent_map[str.charAt(i).toLowerCase()].toUpperCase();
        else
          ret += str.charAt(i);
    }
    return ret;
  ...
  getArticleListBase : function (articleQuery,whereQuery){
    return new Promise(function(resolve){
      ...
      articleQuery.then(function (articles){
            ...
            // articles = _.where(articles,whereQuery);

            // var delta = articles.map(function (obj) {
            //     //
            //     // sails.log(e);
            //     // sails.log(i);
            //     return obj.key;
        ...
        function searchString(str,wstr){
          if(_.isArray(str)) str = JSON.stringify(str);
          return ArticleService._accentFold(str).search(new RegExp(ArticleService._accentFold(wstr), 'i')) > -1;
        }
        function isSuccessfulQuery(arr,cnt){
          return !!(cnt >= (Math.round(_.size(arr)*_querySuccessRate)));
        }

        function matchWord (content,prms) {
          var success = 0;
          if(_.isArray(content)) content = JSON.stringify(content);
          if(_.isArray(prms)){
            _.each(prms,function(pElem){ if(searchString(content,pElem)) success = success +1;});
            return  isSuccessfulQuery(prms,success);
          }
          else{
            return searchString(content,prms);
          }
        }
        ...
        //       {
        //         return (matchWord(aElem[wkElem],whereQuery[wkElem])) ;
        //       }
        //     });
        //   });
        ...
        articles = _.filter(articles,function(aElem){
          ...
          _.each(_.keys(whereQuery),function(wkElem){
            if(_.has(aElem,wkElem))
            {
              if(matchWord(aElem[wkElem],whereQuery[wkElem])) success = success +1;
            }
          });
